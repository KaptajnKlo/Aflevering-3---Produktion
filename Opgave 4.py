...
def buildModel(data: dict) -> pyomo.ConcreteModel():
    ...
    for t in model.periods:
        model.sharedResources.add(
            expr=sum(model.produktionskapacitetforbrug[k]*model.batch[k] * model.x[k, t] for k in model.products)
            <= data['max_prod']
        )
        model.sharedResources.add(
             expr=sum(model.lagerkapacitetforbrug[k]*model.s[k, t] for k in model.products) <= data['max_inv']
        )
